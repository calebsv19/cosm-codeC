...
def run_app(app: Path, initial: Path, resized: Path, log: Path,
    ...
    env["IDE_VULKAN_ROLLOUT_INITIAL_CAPTURE"] = str(initial.resolve())
    env["IDE_VULKAN_ROLLOUT_RESIZED_CAPTURE"] = str(resized.resolve())
    env["IDE_VULKAN_ROLLOUT_MIN_SCALE"] = str(minimum_scale)
    ...
    if result.returncode != 0:
        raise SystemExit(f"IDE Vulkan self-test exited {result.returncode}")
    if "[vk_runtime validation]" in output:
        raise SystemExit("Vulkan validation emitted warning/error diagnostics")
    receipts = re.findall(
        r"IDE_VULKAN_RUNTIME schema=1 stage=(startup|resized|restart) "
    ...
    if not re.search(r"IDE_VULKAN_ROLLOUT schema=1 status=pass\b", output):
