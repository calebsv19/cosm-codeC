...
VkResult vk_renderer_commands_init(VkRenderer* renderer,
    ...
    VkCommandPoolCreateInfo pool_info = {
        ...
        .queueFamilyIndex = renderer->context.graphics_queue_family,
        .flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,
    };

    VkResult result = vkCreateCommandPool(renderer->context.device, &pool_info, NULL,
                                          &out_pool->pool);
    ...
    result = vkAllocateCommandBuffers(renderer->context.device, &alloc_info,
                                      out_pool->buffers);
    if (result != VK_SUCCESS) return result;
    ...
    for (uint32_t i = 0; i < frames_in_flight; ++i) {
        if (vkCreateFence(renderer->context.device, &fence_info, NULL,
                          &out_pool->fences[i]) != VK_SUCCESS) {
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        if (vkCreateSemaphore(renderer->context.device, &semaphore_info, NULL,
...
VkResult vk_renderer_commands_begin_frame(VkRenderer* renderer,
    ...
    vkWaitForFences(renderer->context.device, 1, &frame->in_flight_fence, VK_TRUE,
                    UINT64_MAX);
    vkResetFences(renderer->context.device, 1, &frame->in_flight_fence);

    uint32_t image_index = 0;
    VkResult result = vkAcquireNextImageKHR(renderer->context.device,
