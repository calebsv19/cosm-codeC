...
static VkCommandBuffer begin_single_time_commands(VkRenderer* renderer) {
    ...
    VkCommandBuffer cmd;
    vkAllocateCommandBuffers(renderer->context.device, &alloc_info, &cmd);
...
static void end_single_time_commands(VkRenderer* renderer, VkCommandBuffer cmd) {
    ...
    vkQueueSubmit(renderer->context.graphics_queue, 1, &submit_info, VK_NULL_HANDLE);
    vkQueueWaitIdle(renderer->context.graphics_queue);

    vkFreeCommandBuffers(renderer->context.device, renderer->command_pool.pool, 1, &cmd);
}
...
VkResult vk_renderer_texture_create_from_rgba(VkRenderer* renderer,
    ...
    result = vkCreateSampler(renderer->context.device, &sampler_info, NULL,
                             &out_texture->sampler);
    if (result != VK_SUCCESS) {
    ...
    result = vkAllocateDescriptorSets(renderer->context.device, &alloc_info,
                                      &out_texture->descriptor_set);
    if (result != VK_SUCCESS) {
        vkDestroySampler(renderer->context.device, out_texture->sampler, NULL);
    ...
    vkUpdateDescriptorSets(renderer->context.device, 1, &write, 0, NULL);
...
void vk_renderer_texture_destroy(VkRenderer* renderer,
    ...
    if (texture->descriptor_set) {
        vkFreeDescriptorSets(renderer->context.device, renderer->descriptor_pool, 1,
                             &texture->descriptor_set);
        texture->descriptor_set = VK_NULL_HANDLE;
    }
    if (texture->sampler) {
        vkDestroySampler(renderer->context.device, texture->sampler, NULL);
