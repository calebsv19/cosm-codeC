...
static double rollout_minimum_scale(void) {
    const char *value = getenv("IDE_VULKAN_ROLLOUT_MIN_SCALE");
    char *end = NULL;
...
static int rollout_extent(SDL_Window *window,
    ...
    if (!window || !renderer || !out_extent || !out_scale) return 0;
    SDL_GetWindowSize(window, &logical_width, &logical_height);
    SDL_Vulkan_GetDrawableSize(window, &drawable_width, &drawable_height);
    if (logical_width <= 0 || logical_height <= 0 ||
        drawable_width <= 0 || drawable_height <= 0) return 0;
    ...
    if (!isfinite(scale_x) || !isfinite(scale_y) ||
        fabs(scale_x - scale_y) > 0.01 || scale_x < rollout_minimum_scale()) return 0;
...
static int rollout_verify_runtime(const VkRenderer *renderer, const char *stage) {
    const VkRendererDevice *device;
    const VkRuntimeCapabilityReport *report;
    const char *version;

    if (!renderer || !renderer->context.device) return 0;
    ...
    if (!version || !version[0] || !report ||
        ...
        fprintf(stderr, "IDE_VULKAN_RUNTIME stage=%s status=fail\n", stage);
        return 0;
    }
    printf("IDE_VULKAN_RUNTIME schema=1 stage=%s runtime=%s device=%s "
...
static int rollout_render(VkRenderer *renderer, const char *capture_path) {
    ...
    if (capture_path && vk_renderer_request_capture(renderer, capture_path) != VK_SUCCESS) {
        fprintf(stderr, "IDE_VULKAN_ROLLOUT capture_request=fail\n");
        return 0;
    }
    ...
    if (result != VK_SUCCESS || command_buffer == VK_NULL_HANDLE ||
        ...
        fprintf(stderr, "IDE_VULKAN_ROLLOUT begin_frame=%d\n", (int)result);
        return 0;
    }
    ...
    vk_renderer_set_draw_color(renderer, 0.10f, 0.34f, 0.68f, 1.0f);
    vk_renderer_fill_rect(renderer, &panel);
    vk_renderer_set_draw_color(renderer, 0.96f, 0.78f, 0.12f, 1.0f);
    vk_renderer_draw_line_thick(renderer,
                                ...
                                (float)panel.y,
                                4.0f);
    if (renderer->draw_state.draw_call_count < 2u) return 0;
    result = vk_renderer_end_frame(renderer, command_buffer);
    if (result != VK_SUCCESS) {
        fprintf(stderr, "IDE_VULKAN_ROLLOUT end_frame=%d\n", (int)result);
        return 0;
    }
...
int ide_vulkan_rollout_self_test(void) {
    const char *initial_capture = rollout_capture_path(
        "IDE_VULKAN_ROLLOUT_INITIAL_CAPTURE", "ide-initial.bmp");
    const char *resized_capture = rollout_capture_path(
        "IDE_VULKAN_ROLLOUT_RESIZED_CAPTURE", "ide-resized.bmp");
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) != 0) {
        fprintf(stderr, "IDE_VULKAN_ROLLOUT sdl_init=fail error=%s\n", SDL_GetError());
        return 1;
    }
    window = SDL_CreateWindow("codeC Vulkan Rollout Proof",
                              SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                              800, 500,
                              SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE |
                                  SDL_WINDOW_VULKAN | SDL_WINDOW_ALLOW_HIGHDPI);
    if (!window) {
        fprintf(stderr, "IDE_VULKAN_ROLLOUT window=fail error=%s\n", SDL_GetError());
        goto cleanup;
    }
    ...
    if (vk_renderer_init(&renderer, window, &config) != VK_SUCCESS) {
        fprintf(stderr, "IDE_VULKAN_ROLLOUT renderer_init=fail\n");
        goto cleanup;
    }
    ...
    if (!rollout_verify_runtime(&renderer, "startup") ||
        ...
        !rollout_render(&renderer, initial_capture)) goto cleanup;

    SDL_SetWindowSize(window, 1000, 650);
    SDL_Delay(100u);
    SDL_PumpEvents();
    if (vk_renderer_recreate_swapchain(&renderer, window) != VK_SUCCESS ||
        ...
        !rollout_verify_runtime(&renderer, "resized")) goto cleanup;

    vk_renderer_shutdown(&renderer);
    renderer_initialized = 0;
    if (vk_renderer_init(&renderer, window, &config) != VK_SUCCESS) {
        fprintf(stderr, "IDE_VULKAN_ROLLOUT restart_init=fail\n");
        goto cleanup;
    }
    renderer_initialized = 1;
    if (!rollout_render(&renderer, NULL) ||
        !rollout_verify_runtime(&renderer, "restart")) goto cleanup;

    printf("IDE_VULKAN_ROLLOUT schema=1 status=pass "
           ...
           resized_extent.width, resized_extent.height,
           initial_scale, resized_scale, initial_capture, resized_capture);
    exit_code = 0;

cleanup:
    if (renderer_initialized) vk_renderer_shutdown(&renderer);
