...
WORKSPACE_AUTHORING_PROJECTION_TEST_OUT := $(TEST_BUILD_DIR)/workspace_authoring_projection_test
WORKSPACE_AUTHORING_PROFILE_TEST_OUT := $(TEST_BUILD_DIR)/workspace_authoring_profile_test
...
IDE_UI_BUTTON_ADAPTER_TEST_OUT := $(TEST_BUILD_DIR)/ide_ui_button_adapter_test
EDITOR_CURSOR_PREFERENCES_TEST_OUT := $(TEST_BUILD_DIR)/editor_cursor_preferences_test
EDITOR_GOTO_FRAMING_TEST_OUT := $(TEST_BUILD_DIR)/editor_goto_framing_test
EDITOR_SESSION_CONTROL_PANEL_TEST_OUT := $(TEST_BUILD_DIR)/editor_session_control_panel_test
...
TEST_SMOKE_TARGETS := test-vk-macros test-shared-theme-font-adapter test-ide-ui-button-adapter test-runtime-paths-resolution test-runtime-startup-defaults test-startup-diagnostics test-build-trust-notice test-git-command-runner test-workspace-startup-policy test-workspace-context $(TEST_TERMINAL_TARGETS) test-completed-results-queue test-analysis-scheduler-coalescing test-editor-edit-transaction-debounce test-editor-cursor-preferences test-editor-goto-framing test-editor-session-control-panel test-loop-events-queue test-loop-events-emission-contract test-loop-events-invalidation-policy test-loop-events-dispatch-integration test-fisics-bridge-events-regression test-analysis-store-stamp-regression test-analysis-runtime-events-startup-regression test-analysis-store-published-stamp-regression test-library-index-stamp-regression test-include-graph-snapshot test-idle-efficiency-sanity test-diagnostics-pipeline-integration test-analysis-store-diagnostics-metadata test-diagnostics-artifact-io test-analysis-units-store test-analysis-cache-manifest test-analysis-startup-audit test-include-path-resolver-security test-analysis-incremental-policy test-analysis-token-store-persistence test-analysis-build-graph-store test-analysis-memory-report-store test-analysis-refresh-view test-diagnostic-explanations-cache test-diagnostic-context-cache test-errors-filter test-errors-units-detail test-errors-context-detail test-errors-diagnostic-detail test-control-tree-payload test-control-panel-active-file-provider test-control-panel-composite-tree test-control-panel-units-tree test-control-panel-unit-focus-mode test-tree-row-metrics test-editor-units-projection test-editor-diagnostic-markers test-mainthread-context-scope-regression test-loop-diag-config-regression test-workspace-authoring-profile
