...
release-contract:
	...
	@test "$(RELEASE_PRODUCT_NAME)" = "codeC" || (echo "Unexpected release product"; exit 1)
	@test "$(RELEASE_PROGRAM_KEY)" = "ide" || (echo "Unexpected release program key"; exit 1)
	@test "$(RELEASE_BUNDLE_ID)" = "com.cosm.codec" || (echo "Unexpected release bundle id"; exit 1)
...
release-bundle-audit: release-build
	...
	@otool -L "$(PACKAGE_MACOS_DIR)/ide-bin" > "$(RELEASE_DIR)/otool_ide_bin.txt"
	@otool -L "$(PACKAGE_MACOS_DIR)/idebridge" > "$(RELEASE_DIR)/otool_idebridge.txt"
	@for dylib in "$(PACKAGE_FRAMEWORKS_DIR)"/*.dylib; do \
		[ -f "$$dylib" ] || continue; \
	...
	@"$(PACKAGE_MACOS_DIR)/ide-launcher" --print-config > "$(RELEASE_DIR)/print_config.txt"
	@rg -q '^IDE_RUNTIME_DIR=' "$(RELEASE_DIR)/print_config.txt" || (echo "Missing IDE_RUNTIME_DIR in launcher config"; exit 1)
